...
public class VotingMachinesDeluxe
{
    ...
    public static int p(int N, double a, double f, int T, double erro)
    {
        ...
        if (p <= 0.03 && pComErro > 0.03) return 0;
        else if (p <= 0.03) return 1;
        else return 2;
    }
    ...
    public static double bissecciona2(double inicio, double fim, int N, double a, int T){
        while (inicio <= fim){
            double f = (inicio + fim)/2;
            int p = p(N, a, f, T, 0.001);

            if (p == 0) return f;
            else if(p == 1) inicio = f;
            else fim = f;
        }
